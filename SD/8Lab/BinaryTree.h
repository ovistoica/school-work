...
public:
    ...
    BinaryTree(): pData(nullptr), leftNode(nullptr),
      rightNode(nullptr){
    ...
    int getHeight(BinaryTree<T> *root){
      if(root == nullptr) {
        return 0;
      ...
      int l = 0, r = 0;
      l = 1 + getHeight(root->leftNode);
      r = 1 + getHeight(root->rightNode);
      if(l > r) return l;
      return r;
    }

    // Return the LCA of two nodes in the tree. TODO 2.3
    static BinaryTree<T>* getLCA(BinaryTree<T>* root, BinaryTree<T>* node1,
      BinaryTree<T>* node2) {
